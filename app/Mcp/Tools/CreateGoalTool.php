...
class CreateGoalTool extends IgniteTool
{
     ...
     *
     * Only `title` and `type` are required; operational fields default server-side.
     */
    public function handle(Request $request): ResponseFactory
    {
        $rules = GoalRules::partialRules($this->actor($request));
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'direction' => $schema->string()
                ->description('Whether progress ascends toward or descends to the target. Defaults to ascending.')
                ->enum(['ascending', 'descending'])
                ->nullable(),
            'current_value' => $schema->number()
                ->description('The starting value of the goal. Defaults to 0.')
                ->nullable(),
            'target_value' => $schema->number()
                ->description('The target value. Required for quantifiable goals.')
                ->nullable(),
            'unit' => $schema->string()
                ->description('The unit label for the value, e.g. "km" or "books".')
                ->max(50)
                ->nullable(),
            'status' => $schema->string()
                ->description('The goal status. Defaults to not_started.')
                ->enum(['not_started', 'in_progress', 'completed', 'paused', 'abandoned'])
                ->nullable(),
            'priority' => $schema->string()
                ->description('The goal priority. Defaults to medium.')
            ...
            'polarity' => $schema->string()
                ->description('Whether the habit is positive (build) or negative (quit). Defaults to positive.')
                ->enum(['positive', 'negative'])
                ->nullable(),
            'points' => $schema->integer()
                ->description('Gamification points awarded. Defaults to 0.')
                ->min(0)
                ->nullable(),
            'is_public' => $schema->boolean()
                ->description('Whether the goal is visible to others. Defaults to false.')
                ->nullable(),
            ...
            'deadline' => $schema->string()
                ->description('An optional deadline as YYYY-MM-DD. Must be on or after the start date.')
                ->format('date')
                ->nullable(),
            'order' => $schema->integer()
                ->description('Display order; if omitted the server assigns the next position.')
                ->nullable(),
        ];
    }
}
