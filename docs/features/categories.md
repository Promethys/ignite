...
- On the goals list, filtering by category happens client-side: the page loads the user's full goal list, and the filter selection is held in the `?category=` query parameter in both directions. Arriving at `/goals?category={id}` opens the list already filtered to that category, and changing the filter updates the URL in place, so a reload, a bookmark, or a link shared with someone else lands on the same view. The uncategorised option serialises as `?category=none`, and an absent parameter means all categories. None of this triggers a new request to the server.
- The status filter and the search box on the same page work the same way, through `?status=` and `?search=`. A filter left at its default is omitted, so an untouched list keeps a clean `/goals`. The three parameters combine, and the goal entries list applies the same idea with `?search=`, `?from=` and `?to=`.
